...
public class UserCtl extends BaseCtl {
	...
	public String submit(@RequestParam(required = false) Long id, @ModelAttribute("form") @Valid UserForm form,
		...
		if (OP_SAVE.equalsIgnoreCase(form.getOperation())) {
			...
			/*
			 * if(!dto.getPassword().equals(dto.getConfirmPassword())) { String
			 * msg=messagesource.getMessage("pattern.matchpass",null,locale);
			 * model.addAttribute("error",msg); return "AddUser"; }
			 * 
			 */ if (id > 0) {
				service.update(dto);
